...
class HubCoordinator(DataUpdateCoordinator[dict[str, ResolvedValue]]):
    ...
    async def async_start(self) -> None:
        ...
        # before the summary: on a fresh install the config flow has just
        # introduced the estimates, so only an upgrade deserves a notice
        await self._maybe_notify_derived()
    ...
    async def _maybe_notify_derived(self) -> None:
        """Announce the battery estimates once, on the upgrade that adds them."""
        if not self.derived_available:
            return
        if self._store_data.get("derived_notified"):
            return
        if self._store_data.get("summary_shown"):
            self._notify(f"{DOMAIN}_derived", get_texts(self.hass)["derived"])
        self._store_data["derived_notified"] = True
        await self._store.async_save(self._store_data)
