...
- **Battery runtime estimates** (optional, on by default): estimated time to a full charge and time down to a reserve SoC, calculated from data the hub already aggregates — no extra polling.
...
| Entity | Meaning |
| ------ | ------- |
| `sensor.hf_hub_battery_time_to_full` | Minutes to 100% SoC. Unknown while discharging or idle. |
| `sensor.hf_hub_battery_time_to_empty` | Minutes down to the reserve SoC. Unknown while charging or idle. |
...
| Reserve state of charge | 5% | The discharge estimate counts down to this, not to zero. Match it to the end-of-discharge SoC set in your inverter |
